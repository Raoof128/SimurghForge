        ...
        <div className="mt-2.5 space-y-2.5 animate-fade-in">
          {category === "image" && options.image && (
            <>
              ...
              <ControlRow
                label="Max Width"
                value={options.image.maxWidth ? `${options.image.maxWidth}px` : "Original"}
              >
                <input
                  type="range"
                  min={0}
          ...
          {category === "video" && options.video && (
            <>
              ...
              <ControlRow
                label="Bitrate"
                value={options.video.bitrate ? `${options.video.bitrate} kbps` : "Auto"}
              >
                <input
                  type="range"
                  min={500}
          ...
          {category === "data" && options.data && (
            <>
              {(outputFormat === "csv" || outputFormat === "tsv") && (
                <ControlRow
                  label="Delimiter"
                  value={options.data.delimiter === "\t" ? "Tab" : (options.data.delimiter ?? ",")}
                >
                  <select
...
function ControlToggle({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <div
          className={`absolute top-0.5 w-3 h-3 rounded-full transition-all duration-200 ${checked ? "left-4 bg-accent" : "left-0.5 bg-text-muted/40"}`}
        />
      </button>
    </div>
  );
